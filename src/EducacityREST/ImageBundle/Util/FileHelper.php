...
namespace EducacityREST\ImageBundle\Util;
...
class FileHelper
{
    const PATH = "/var/www/educacity/web/uploads";
    ...
    public static function getFileNameFromId($id, $name)
    {
        $fileName = $id . '-' . $name;
    ...
    public static function removeFile($obj, $subdirectory, $idObjectoPrincipal, $subNombreArchivo = null)
    {
        $path = self::PATH . '/' . $subdirectory . '/'
            . self::getDirectoryNameFromId($idObjectoPrincipal) . '/'
            . $obj;
        @unlink($path);
    }

    public static function executeConvert($path, $originalFilePath, $width, $height, $quality)
    {
        ...
        $dim = self::getResizeValues($size[0], $size[1], $width, $height);
        if ($scale <= 0) {
     ...
     */
    public static function getResizeValues($originalWidth, $originalHeight, $maxWidth, $maxHeight)
    {
        $width = $originalWidth;
    ...
    public static function resizeAndSaveImage($image, $width, $height, $directory, $sufix, $maximize=false)
    {
        ...
        list($_width, $_height) = self::intelligentResizeThumbnail($width, $height, $originalWidth, $originalHeight, $maximize);
    ...
    public static function intelligentResizeThumbnail($width, $height, $originalWidth, $originalHeight, $maximize=false)
    {
        if ($maximize) {
